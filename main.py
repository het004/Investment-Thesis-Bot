...
from INVESTMENT_THESIS.entity.config import (
    PipelineConfig, DataIngestionConfig, DataStorageConfig,
    SentimentAnalysisConfig, RecommendationConfig, ReportConfig
)
...
# Optional: allow CORS if calling from external frontends
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def run_pipeline(ticker: str) -> dict:
    try:
        ...
        cached = collection.find_one({"ticker": ticker, "generated_at": {"$gte": today}})
        if cached:
            report = report_generator.generate_report(
                ticker, {}, [], [], {}, {"rsi": None}, cached["recommendation"]
            )
            last_update = cached["generated_at"].astimezone(ist).strftime("%I:%M %p IST")
        ...
        # Store
        logging.info("Storing fetched data")
        ...
        recommendation = recommendation_engine.generate_recommendation(
            company_data, technicals, news_sentiment, social_sentiment
        )
